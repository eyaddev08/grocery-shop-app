...
class CategoriesCubit extends Cubit<CategoriesState> {
  ...
  Future<void> loadCategories() async {
    ...
    final Either<Failure, List<CategoryEntity>> res = await getCategories();
    if (isClosed) return;
    res.fold((l) => emit(CategoriesError(l.message)), (r) {
      _allCategorys = List<CategoryEntity>.from(r);
      filterIndex = 0;
      emit(CategoriesLoaded(List<CategoryEntity>.from(_allCategorys)));
    });
  }

  int filterIndex = 0;

  List<CategoryEntity> _allCategorys = [];

  List<String> get filters {
    final List<String> labels = [];
    for (final p in _allCategorys) {
      final lbl = p.tag.trim();
  ...
  Future<void> filterCategory(int idx) async {
    ...
    final filtered = _allCategorys.where((p) => p.tag == label).toList();
