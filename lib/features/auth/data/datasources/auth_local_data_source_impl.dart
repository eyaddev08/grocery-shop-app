// تنفيذ مصدر بيانات المصادقة المحلية
...
class AuthLocalDataSourceImpl implements AuthLocalDataSource {
  ...
  Future<void> saveTokens(AuthTokenModel token) async {
    await storage.write(key: SecureStorageKeys.accessToken, value: token.accessToken);
    await storage.write(key: SecureStorageKeys.refreshToken, value: token.refreshToken);
    await storage.write(key: SecureStorageKeys.tokenExpiresIn, value: token.expiresIn.toString());
    await storage.write(key: SecureStorageKeys.tokenType, value: token.tokenType);
  }
  ...
  Future<AuthTokenModel?> getTokens() async {
    ...
    final expiresInStr = await storage.read(key: SecureStorageKeys.tokenExpiresIn);
