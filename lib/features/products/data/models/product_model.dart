...
class ProductModel {
  ...
  // parse filterLabel — حاول مفاتيح مختلفة حسب API
    ...
    final originalPrice =
        json.containsKey('purchase_price') ? parseDouble(json['purchase_price'], null) : null;
    final discount =
        json.containsKey('discount') ? parseDouble(json['discount'], null) : null;
    ...
    final nutritionLines = parseList<String>(json['nutritionLines'], (e) => e.toString());

    // thumbnail: ensure we produce String? (never a raw dynamic)
    final dynamic rawThumb = json['thumbnail_full_url'] ?? json['thumbnail'] ?? (images.isNotEmpty ? images.first : null);
    ...
    final dynamic rawShort = json['short_description'] ?? json['details'] ?? json['description'];
    ...
    final rawCat = json['category_ids'] ?? json['categories'] ?? json['category_ids_formatted'];
    if (rawCat != null) {
      if (rawCat is List) {
